...
{
    [System.Web.Mvc.RoutePrefix("api/Instagram")]
    ...
    { // GET: Profiles
        ...
        public List<Profile> Get()
        {
            ...
            foreach (var extended in entities.Misper_Extended.Where(x => !string.IsNullOrEmpty(x.InstagramName)))
            {
        ...
        private Profile ReadProfile(misper_ person)
        {
            ...
            profile.InstagramName = extended.InstagramName;
        ...
        public List<Instagram> Get(string uniqueID)
        {
            List<Instagram> retInstagrams = new List<Instagram>();
            GMPRestApi.Models.Data.GMPMissingPersonEntities entities = new GMPRestApi.Models.Data.GMPMissingPersonEntities();
            var person = entities.misper_.Where(x => x.Unique_ID == uniqueID).FirstOrDefault();
            if (person != null)
            {
                var extended = person.Misper_Extended.Where(x=>!string.IsNullOrEmpty(x.InstagramName)).FirstOrDefault();
                if (extended != null)
                {
                    System.Net.WebClient webClient = new System.Net.WebClient();
                    dynamic results = JObject.Parse(webClient.DownloadString("https://www.instagram.com/" + extended.InstagramName + "/media/"));
                    DateTime starTime = DateTime.Now;
                    foreach (dynamic item in results.items)
                    {
                        if (item.type == "image")
                        {
                            int maxHeight = 0;
                            starTime = starTime.AddHours(-1);
                            Instagram instagram = new Instagram();
                            instagram.DateTime = starTime;
                            instagram.InstagramName= extended.InstagramName;
                            instagram.UserID = uniqueID;
                            instagram.PostUrl = item.link;
                            foreach (dynamic image in item.images)
                            {
                                    instagram.ImageURL = image.Value.url.Value;
                            }
                            retInstagrams.Add(instagram);
                            if (retInstagrams.Count > 10)
                            {
                                break;
                            }
                        }
